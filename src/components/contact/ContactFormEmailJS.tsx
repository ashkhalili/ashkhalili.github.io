  ...
  return (
    <form key={formKey} ref={formRef} onSubmit={handleSubmit} noValidate className="glass rounded-xl p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div><label htmlFor="ej_name" className={lbl}>Full Name <span className="text-red-500">*</span></label>
          <input id="ej_name" name="name" type="text" required autoComplete="name" placeholder="Jane Smith" className={input} /></div>
        <div><label htmlFor="ej_email" className={lbl}>Email <span className="text-red-500">*</span></label>
          <input id="ej_email" name="email" type="email" required autoComplete="email" placeholder="jane@example.com" className={input} /></div>
        <div><label htmlFor="ej_phone" className={lbl}>Phone <span className="font-normal text-txt-s dark:text-darkmode-txt-s">(optional)</span></label>
          <input id="ej_phone" name="phone" type="tel" autoComplete="tel" placeholder="+44 7xxx xxxxxx" className={input} /></div>
        <div><label htmlFor="ej_enquiry" className={lbl}>Enquiry Type <span className="text-red-500">*</span></label>
          <select id="ej_enquiry" name="enquiry" required value={enquiry} onChange={e => setEnquiry(e.target.value)} className={input}>
      ...
      {isPostOp && (
        <div className={sec}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div><label htmlFor="ej_procedure" className={lbl}>Procedure <span className="text-red-500">*</span></label>
              <select id="ej_procedure" name="procedure" required defaultValue="" className={input}>
                <option value="" disabled>Select…</option>
            ...
            <div><label htmlFor="ej_date" className={lbl}>Approximate Date <span className="text-red-500">*</span></label>
              <input id="ej_date" name="procedure_date" type="month" required className={input} /></div>
          </div>
          ...
          <div className="space-y-5">
            <div><label htmlFor="ej_concerns" className={lbl}>Concerns during recovery?</label>
              <textarea id="ej_concerns" name="recovery_concerns" rows={3} placeholder="e.g. pain, redness, visual disturbance…" className={`${input} resize-none`} /></div>
            <div><label htmlFor="ej_well" className={lbl}>What went particularly well?</label>
              <textarea id="ej_well" name="went_well" rows={3} className={`${input} resize-none`} /></div>
            <div><label htmlFor="ej_improve" className={lbl}>What could we improve?</label>
              <textarea id="ej_improve" name="improvements" rows={3} className={`${input} resize-none`} /></div>
          </div>
        </div>
      )}
      ...
      {/* Message */}
      <div className={sec}>
        <label htmlFor="ej_message" className={lbl}>
          {isPostOp ? "Any further comments" : <>Message <span className="text-red-500">*</span></>}
        </label>
        <textarea id="ej_message" name="message" required={!isPostOp} rows={4}
      ...
      <div className="mt-6 mb-8 flex items-start gap-3">
        <input id="ej_consent" name="consent" type="checkbox" required className="mt-0.5 h-4 w-4 shrink-0 rounded accent-slate-700" />
        <label htmlFor="ej_consent" className="text-xs text-txt-s dark:text-darkmode-txt-s leading-relaxed">
